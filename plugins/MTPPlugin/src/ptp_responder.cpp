...
namespace haze {
    ...
    Result PtpResponder::Initialize(EventReactor *reactor, PtpObjectHeap *object_heap) {
        ...
        /* INI partitions plus the built-in Album (upstream never called */
        /* LoadCustomPartitions from Initialize, so this was dead code). */
     ...
     * INI config loader.
     *
     * Reads sdmc:/qlaunch-ext/config/plugin/mtp.ini and populates
     * m_custom_partitions, then the built-in Album follows the INI entries.
     *
     * Expected format:
     *   [Partition Label]
    ...
    void PtpResponder::LoadCustomPartitions() {
        ...
        /* Built-in Album partition, right after the SD card: screenshots */
        /* and recordings live on the SD card itself, so expose them as   */
        /* their own storage like sphaira does.                           */
        {
            ...
            std::strncpy(entry.name, "Album", sizeof(entry.name));
            entry.name[sizeof(entry.name) - 1] = 0;
            std::strncpy(entry.root_path, "/Nintendo/Album", sizeof(entry.root_path) - 1);
            entry.root_path[sizeof(entry.root_path) - 1] = 0;
            ++m_custom_partition_count;
        }

        FILE *f = fopen("sdmc:/qlaunch-ext/config/plugin/mtp.ini", "r");
        if (f == nullptr) {
            return;
        }
